...
class Gkp:
    ...
    def random_neighbor(self, k: int, i, top_mask):
        if isinstance(i, np.ndarray):
            # y = np.zeros(len(i))
            y = np.array(
                [np.random.choice(self.neighbors[k][i[j]], 1)[0] if top_mask[j] == 1 else -2 for j in range(len(i))])
            return y
...
class RSS2(RSS):
    ...
    def degree_prop_state_sample(self, k, n_samples, top_mask=None) -> np.ndarray:
        ...
        curr_s = np.array(
            [self.Gk.state_merge(k - 1, (u[i], v[i])) if top_mask[i] == 1 else -1 for i in range(n_samples)],
            dtype=np.int)
        ...
        for _ in range(self.t_k(k)):
            ...
            next_s = np.array(
                [self.Gk.state_merge(k - 1, (u[i], v[i])) if loop_mask[i] == 1 else -1 for i in range(n_samples)],
                dtype=np.int)
